...
*/
    ...
    "Balances", {"BALAMT"}),
     #"Added GL Numbers" = Table.ExpandTableColumn(
        Table.NestedJoin(#"Added Balances", {"BRANCHORGNBR", "MJACCTTYPCD", "CURRMIACCTTYPCD", "BALCATCD", "BALTYPCD"},
          DNAGLMapping, {"COSTCENTER", "MJACCTTYPCD", "MIACCTTYPCD", "BALCATCD", "BALTYPCD"},
        "GL", JoinKind.LeftOuter),
    "GL", {"GLNUM", "GLACCTTITLENAME"}),
    #"Filtered Rows" = Table.SelectRows(#"Added GL Numbers",
      each ([CURRACCTSTATCD] <> "CO" and [CURRACCTSTATCD] <> "NPFM")),
    #"Grouped Rows" = Table.Group(#"Filtered Rows", {"GLNUM", "BRANCHORGNBR"},
      {{"Product Balance", each Number.Abs(List.Sum([BALAMT])), type number}})
