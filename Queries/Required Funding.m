...
//Account balances are added from the SubAcctBalances query.  These balances are what DNA considers principal only balances
    #"Added Balances" = Table.ExpandTableColumn(
        Table.NestedJoin(ABCUSource, {"Source DB", "ACCTNBR", "SUBACCTNBR"}, 
            SubAcctBalances, {"Source DB", "ACCTNBR", "SUBACCTNBR"}, 
        "Balances", JoinKind.LeftOuter),
    "Balances", {"BALAMT"}),

//IFRS 9 reporting must be completed prior to starting NCCF and NSFR.  During IFRS 9 reporting a report is generated that provides loan stages, 
//ECL amounts (on an account level)
//That information is imported into this workbook and used new row is created with the loan stage.
    #"Added loan stages" = Table.ExpandTableColumn(
        Table.NestedJoin(#"Added Balances", {"Source DB", "ACCTNBR"}, IFRS9LoanStages, {"Source DB", "Account Number"}, "Stages", JoinKind.LeftOuter),
...
//The following is a basic IF statement that looks at loan stages, product type and remaining amortization to determine the correct line for reporting.  
//If none of the statements are met an error code of 9999 is returned
